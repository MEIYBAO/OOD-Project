...
async function fetchList() {
  ...
  try {
    ...
    // 兼容不同封装：优先 { data, total }
    userList.value = res.data || res?.data?.data || res?.data || []
    total.value = res.total ?? res?.data?.total ?? 0
  } catch (e: any) {
    console.error('[getUsers error]', e?.response?.status, e?.response?.data)
    ElMessage.error(e?.response?.data?.message || '获取用户失败')
  } finally {
    loading.value = false
  }
}
...
function handleEdit(row: any) {
  ...
  // 后端返回 { id: username, username, role }
...
async function handleDelete(row: any) {
  try {
    ...
    await deleteUser(row.id) // id 等于 username
  ...
  formRef.value?.validate(async (valid: boolean) => {
    ...
    try {
      if (form.id) {
        ...
        await updateUser(form.id, payload)                   // id = username
      ...
      console.error('[save user error]', e?.response?.status, e?.response?.data)
